...
DEFAULTS: dict = {
    ...
    # הסמלים אומתו מול עץ אמיתי באתר (תבנית:עץ משפחת בלוי): ! אנכי, - אופקי, ~ קו נישואין,
    # ד צומת נישואין (עם קו למטה), ז צומת של ילד באמצע, + הצטלבות, . פינת הילד הראשון (מימין), , פינת האחרון.
    # סמלים שלא אומתו (t_up, t_right, t_left, up_*) המחולל נמנע מהם; אם התבנית תומכת בהם – לעדכן כאן.
    "chart": {
        ...
        "symbols": {
            ...
            "marriage": "ד", "marriage_h": "~",
        },
        "mirror_horizontal": False,
        "box_years": True,
        "references_tag": "<references />",
        # בני זוג של צאצאים: "inline" = בתוך הקופסה ("שם, אשת [[בעלה]]") כמקובל באתר; "boxes" = קופסה נפרדת
        "spouse_style": "inline",
        "root_spouse_boxes": True,
        "show_sons_wives": True,
        # מילים לתיאור בן/בת הזוג בתוך הקופסה
